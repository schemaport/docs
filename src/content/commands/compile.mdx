...
`Result:` line counts the whole run.

Each `•` line is one [transformation](/concepts/safe-and-lossy-compilation),
tagged `[safe]` or `[lossy]`, with the canonical schema path it applied to. The
`⚠` lines are diagnostics that survived compilation — things the target accepts
but represents or enforces differently.
...
OpenAI
✓ generated/openai/tag-resource.json
  • [safe] renamed-input-schema-to-parameters at inputSchema
    Emitted `inputSchema` as the OpenAI `parameters` field.
  • [safe] enabled-strict-mode at inputSchema
    Emitted `strict: true` so OpenAI enforces the schema instead of best-effort matching.
...
Gemini
✓ generated/gemini/tag-resource.json
  • [lossy] dropped-additional-properties at inputSchema.properties.tags.additionalProperties
    Dropped `additionalProperties`; Gemini has no such field, so extra properties are accepted.
  • [safe] renamed-input-schema-to-parameters at inputSchema
    Emitted `inputSchema` as `FunctionDeclaration.parameters`.
