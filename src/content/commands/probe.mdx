...
|---|---|---|
| `--targets <ids>` | `openai,anthropic,gemini` | Which targets to probe. MCP is excluded by default because it has no hosted API; naming it explicitly reports it as skipped. `all` adds MCP, which reports `skipped`. |
...
|---|---|---|
| `openai` | `SCHEMAPORT_OPENAI_MODEL` | `gpt-5.6-terra` |
| `anthropic` | `SCHEMAPORT_ANTHROPIC_MODEL` | `claude-haiku-4-5` |
| `gemini` | `SCHEMAPORT_GEMINI_MODEL` | `gemini-2.5-flash-lite` |

The defaults are small, inexpensive models — a probe only needs the provider to
parse the tool definition, not to reason at length.
