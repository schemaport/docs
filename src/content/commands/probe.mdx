...
## The four verdicts
When a tool call comes back, its arguments are validated against your canonical input schema. This validation checks if the provider accepted the schema and then ignored any constraints. If the validation fails, the result carries `argumentsValid` as `false` and `argumentErrors` detailing the discrepancies. This ensures that any provider that accepts a schema but then ignores its constraints is reported as such, rather than being incorrectly marked as passing validation.
<!-- thally-cite:v1:c7f8ba32862e82f608f2f46722ef9dc948b92052f58203cdb301aa0e96e5d9f6 -->
