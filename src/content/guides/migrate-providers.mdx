...
<Badge variant="danger">1 error</Badge>
...
<Note>
  SchemaPort emits the default, non-strict form unless you ask for enforcement.
  The strict subset returns a 400 for `minimum`, `maximum`, `multipleOf`,
  `minLength`, `maxLength` and several array constraints, so emitting it means
  dropping those keywords — lossy under SchemaPort's rules.

  The choice is explicit rather than hidden. By default the whole schema is sent
  and `check` tells you none of it is enforced. When you want enforcement, pass
  `strict: true` and `allowLossy` together:

  ```ts
  compileTool(tool, { strict: true, allowLossy: true })
  ```

  Every dropped keyword is recorded as a lossy transformation, so you can see
  exactly which constraints you gave up. Strict mode is a library option — the
  CLI has no `--strict` flag.
</Note>
