...
in `CanonicalTool`, so they are supplied per call rather than read off the tool:
...
with no useful error. Without a `metadata` option the output is exactly the
three-field form shown above.
...
They validate the supplied MCP fields, not the canonical tool.
...
<Note>
**MCP has no lossy path.** The three root-type transformations make the type at
least as strict as it was, and `attached-tool-metadata` only adds fields —
nothing in the canonical schema is removed or weakened by declaring an
`outputSchema` or a `title`. No `minimum`, `pattern`, `enum`,
...
Compilation is refused — `ok: false`, no output — for the four input-schema
errors compile cannot work around (`mcp/input-schema-not-object` and the three
`x-mcp-header` errors) and for the five metadata errors listed in the metadata
rules table above. The blocking diagnostics stay in the result so you can see
why, and `--allow-lossy` does not override any of them; they are not lossy
...
<AccordionGroup>
  ...
  <Accordion title="Metadata is per call, not per tool file">
    `title`, `outputSchema`, `annotations`, `icons` and `_meta` are supplied as
    a compile option, not stored in the canonical tool. A `.json` tool
    definition cannot record its own `outputSchema`, so a pipeline that loads
    tools from disk has to keep the metadata alongside them and pair the two up
    itself. The `schemaport` CLI does not do that pairing:
    `schemaport compile --targets mcp` emits the three-field form.
  </Accordion>
