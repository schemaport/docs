...
All 30 codes are prefixed `openai/`, and each carries a path and a `docsUrl`
...
size limit would mean deleting part of your contract. Three further
**near-limit** rules fire *below* the ceiling and refuse nothing — they warn
when a budget is above 90% of its limit so you have time to act before the next
addition triggers a rejection.

| Code | Severity | Compile | What it checks |
| --- | --- | --- | --- |
...
Each near-limit warning states the headroom left. At exactly the limit it says
"There is no headroom left: any addition will make OpenAI reject the tool."
Each is branched off its own refusal, so a schema over the limit gets the error
alone — never two findings about the same number.

The 90% threshold is exported as `SIZE_WARNING_FRACTION` so it can be read
rather than guessed at. It is a judgement, not a figure OpenAI documents.

`openai/large-enum-too-long` has no near-limit warning: it is a conditional
rule about one enum rather than a budget accumulating across the schema, so
there is no single number to be close to.

The near-limit warnings compile unchanged — `compile.supported` is `true`,
`compile.lossy` is `false`, there is no accompanying `Transformation`, and
`--allow-lossy` is irrelevant.
...
export SCHEMAPORT_OPENAI_MODEL=gpt-5.6-terra  # optional, overrides the default
...
supports function calling and structured outputs while providing more reasoning
headroom than the lowest-cost model.
...
| Models — `gpt-5.6-terra` | https://developers.openai.com/api/docs/models/gpt-5.6-terra |
