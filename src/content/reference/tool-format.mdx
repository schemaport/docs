...
| References | `$ref`, `$defs`, `definitions` — same-document references are resolved; external, dangling and recursive ones are reported, not followed |
...
<AccordionGroup>
  <Accordion title="Only same-document $ref is resolved">
`$ref` pointing inside the same document is resolved, so a constraint that only
exists behind a reference is genuinely checked rather than skipped.

A reference that **cannot** be resolved — external, dangling or recursive — is
reported rather than followed: value validation says the value could not be
verified, and why, instead of returning a pass. Everything around it is still
checked, so one opaque pointer does not make the whole value unverifiable.

**Recursive schemas remain unsupported.** Recursion is detected and named as the
reason rather than being followed until something breaks.
  </Accordion>
...
These are not part of the supported subset. `if`/`then`/`else` and
`dependentSchemas` are ignored during value validation. `format` is not
enforced — providers treat it as advisory.
  </Accordion>
