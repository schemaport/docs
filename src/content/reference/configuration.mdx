...
Default target ids. Replaces `--targets`. Accepts `"all"` as a shorthand for
every target. Validated exactly like the flag: an unknown id exits `2` and lists
the valid ids, and `"all"` expands the same way.
</ParamField>
